...
def buildExamplesFromExtraData(testSize=0.25):

    def trainTestSplit(examples, testSize):
        random.shuffle(examples)
        splitIndex = int(len(examples) * testSize)
        return examples[:splitIndex], examples[splitIndex:]

    with open('iris.data', 'r') as fh:
        lines = fh.read()
        inputs = lines.strip().replace("\n", ",").split(",")
    examples = []
    for i in range(0, len(inputs), 5):
        feature1 = float(inputs[i])
        feature2 = float(inputs[i + 1])
        feature3 = float(inputs[i + 2])
        feature4 = float(inputs[i + 3])
        labelString = inputs[i + 4]
        if labelString == "Iris-setosa":
            ...
            label = [0, 0, 1]
        examples.append(([feature1, feature2, feature3, feature4], label))
    return trainTestSplit(examples, testSize)
