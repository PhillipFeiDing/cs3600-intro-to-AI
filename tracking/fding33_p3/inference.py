    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.beliefs:
            newPosDict = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos in newPosDict.keys():
                prob = self.beliefs[oldPos] * newPosDict[newPos]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for particle in self.particles:
            weights[particle] = weights[particle] + emissionModel[util.manhattanDistance(pacmanPosition, particle)]
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            if oldPos not in cachedDistribution.keys():
                cachedDistribution[oldPos] = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            particles.append(util.sample(cachedDistribution[oldPos]))
        self.particles = particles
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        product = [item for item in itertools.product(self.legalPositions, repeat=self.numGhosts)]
        ...
        for particle in self.particles:
            ...
            for i in range(len(particle)):
                if noisyDistances[i] is not None:
                    ...
                    prob = prob * emissionModels[i][util.manhattanDistance(pacmanPosition, ghostPosition)]
                else:
                    particle = self.getParticleWithGhostInJail(particle, i)
            distribution[particle] = distribution[particle] + prob

        if distribution.totalCount() == 0:
            self.initializeParticles()
            for ghostIdx in range(self.numGhosts):
                if noisyDistances[ghostIdx] is None:
                    for particleIdx in range(len(self.particles)):
                        self.particles[particleIdx] = self.getParticleWithGhostInJail(self.particles[particleIdx],
                                                                                      ghostIdx)
        else:
            self.particles = [util.sample(distribution) for _ in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                newParticle[i] = util.sample(getPositionDistributionForGhost(setGhostPositions(gameState, newParticle),
                                                                             i, self.ghostAgents[i]))
            "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        weights = util.Counter()
        for particle in self.particles:
            weights[particle] = weights[particle] + 1
        weights.normalize()
        return weights
        # util.raiseNotDefined()
